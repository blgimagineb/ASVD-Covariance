...
def remove_scaling_attributes(model):
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear) and ("k_proj" in name or "v_proj" in name):
            ...
            if hasattr(module, 'scaling_inverse'):
...
def main(args):
    ...
    if not args.raw_model:
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Linear) and ("k_proj" in name or "v_proj" in name):
                if "covariance" in args.scaling_method:
                    module.scaling_diag_matrix = matrix_alpha(module.scaling_diag_matrix, args.alpha).to(torch.float32)
                    module.scaling_inverse = matrix_alpha(module.scaling_diag_matrix, -1).to(torch.float32).to(module.weight.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="abs_mean",
        choices=["abs_mean", "abs_max", "fisher", "fisher_abs_mean", "covariance"],
        help="scaling method",
    )
    parser.add_argument(
        "--sensitivity_metric",
