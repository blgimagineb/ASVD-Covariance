...
class SVDLinear(nn.Module):
    ...
    def from_linear(
        ...
        rank = int(linear.out_features * param_ratio)
        ...
        if act_aware:
            ...
            if hasattr(linear, "fisher_info"):
                scaling_diag_matrix *= linear.fisher_info
                # scaling_diag_matrix *= linear.fisher_info**1
            # if not (scaling_diag_matrix == scaling_diag_matrix).all():
            #     breakpoint()
             # avoid zero division
            if(len(scaling_diag_matrix.shape)==2):
                scaling_diag_matrix = linear.scaling_diag_matrix
                w = torch.mm(w, scaling_diag_matrix)
            else:
        ...
        if act_aware:
            if(len(scaling_diag_matrix.shape)==2):
                scaling_diag_matrix = linear.scaling_inverse
                V = torch.mm(scaling_diag_matrix,V)
            else:
